					...
					Promise.all(tableList.map(async (aTable) => {
						...
						.then(rs => {
							const output = rs.map((elem, index) => {
								...
								let primary_query = "SELECT DISTINCT Col.Column_Name,Tab.TABLE_SCHEMA,Col.Table_Name FROM information_schema.TABLE_CONSTRAINTS AS Tab INNER JOIN information_schema.KEY_COLUMN_USAGE AS Col ON Col.Constraint_Name = Tab.Constraint_Name AND Col.Table_Name = Tab.Table_Name AND Col.Table_schema = Tab.Table_schema AND Tab.Constraint_Type = 'PRIMARY KEY' AND Col.Table_Name = 'sinh_vien' AND Col.Table_schema = " + conn.connection.config.database;
...
const syncTable = function (srConfig, desConfig, mapConfig) {
	mysql.createConnection(srConfig)
		.then((srConn) => {
			...
			srConn.query('SHOW TABLES')
				.then((result) => {
					let tableList = result[0].map((elem) => elem['Tables_in_' + srConn.connection.config.database])
					Promise.all(tableList.map((elem) => srConn.query('SHOW CREATE TABLE ' + elem)))
						.then((values) => {
							const tableDefs = values.map((elem) => elem[0][0]['Create Table'])
							mysql.createConnection(desConfig)
								.then((desConn) => {
									desConn.query('SET AUTOCOMMIT=0')
